...
class TestController {
    ...
    async updateShowGradeOutside(req, res) {
        try {
            const { id } = req.params;
    ...
    async getAvailableTests(req, res) {
        try {
            ...
            const allStartTimes = (tests || []).map((t) => ({
                ...
                start_time_cairo: t.start_time_utc
                    ? new Date(t.start_time_utc).toLocaleString("en-US", {
                        timeZone: "Africa/Cairo",
                    })
                    : null,
    ...
    async gradePhysicalBatch(req, res) {
        try {
            ...
            const namesAsIds = (() => {
                const raw = req.body?.names_as_ids ??
                    req.body?.namesAsIds ??
    ...
    async updateBubbleAnswers(req, res) {
        try {
            ...
            const teacherComment = typeof req.body?.teacher_comment === "string"
    ...
    async regradeAllSubmissions(req, res) {
        try {
            const { id } = req.params;
            if (!id) {
                res.status(400).json({ message: "Test ID is required" });
    ...
    async regradePhysicalSubmission(req, res) {
        try {
            const { id } = req.params;
            if (!id) {
                res.status(400).json({ message: "Submission ID is required" });
