...
const checkGhostscript = async (): Promise<{ available: boolean; version?: string; error?: string }> => {
  try {
...
export const getTestPage = async (req: Request, res: Response): Promise<void> => {
  const requestId = uuidv4();
...
// Get test page count
export const getTestPageCount = async (req: Request, res: Response): Promise<void> => {
  const requestId = uuidv4();
