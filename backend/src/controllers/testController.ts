...
const handleArrayUpload = (req: Request, res: Response, next: NextFunction) => {
	try {
		...
		const imageFields = Object.keys(req.body).filter(
			(key) => key.startsWith("images[") && key.endsWith("]"),
		);
		...
		if (imageFields.length > 0) {
			...
			imageFields.forEach((field) => {
				...
				if (file && typeof file === "object" && "originalname" in file) {
					(req.files as Express.Multer.File[]).push(
...
class TestController {
	...
	async createTest(req: AuthenticatedRequest, res: Response): Promise<void> {
		...
		const testData =
			typeof req.body.testData === "string"
		...
		try {
			...
			if (typeof view_permission !== "boolean") {
				return res.status(400).json({
					message: "view_permission is required and must be a boolean",
				});
			}

			const updatedTest = await testService.updateViewPermission(
				Number(id),
				view_permission,
			);
	...
	async updateShowGradeOutside(
		...
		try {
			...
			if (typeof show_grade_outside !== "boolean") {
				return res.status(400).json({
					message: "show_grade_outside is required and must be a boolean",
				});
			}
	...
	async gradeSubmission(
		...
		try {
			...
			const submission = await testService.gradeSubmission(
				submissionId,
				score,
				teacher_comment,
			);
	...
	async getSubmissionWithTest(
		...
		try {
			const { id, submissionId } = req.params as {
				id: string;
				submissionId: string;
			};
			if (!id || !submissionId) {
				res
					.status(400)
					.json({ message: "Test ID and Submission ID are required" });
				return;
			}
			const testId = parseInt(id, 10);
	...
	async setManualGrades(
		...
		try {
			...
			const { grades, teacher_comment } = req.body as {
			...
			const updated = await testService.setManualGrades(
				submissionId,
				grades,
				teacher_comment,
			);
	...
	async getAvailableTests(
		...
		try {
			...
			const cairoNowString = utcNow.toLocaleString("en-US", {
			...
			const allStartTimes = (tests || []).map((t) => ({
				...
				start_time_cairo: (t as any).start_time_utc
					? new Date((t as any).start_time_utc).toLocaleString("en-US", {
							timeZone: "Africa/Cairo",
						})
					: null,
	...
	async uploadBubbleSheet(
		...
		try {
			...
			if (isNaN(testId) || !req.user?.id || !bubbleSheetFile) {
				res
					.status(400)
					.json({ message: "Invalid test ID, user ID, or missing file" });
				return;
			}

			const submission = await testService.uploadBubbleSheet(
				testId,
				req.user.id,
				bubbleSheetFile.path,
			);
			if (!submission) {
	...
	async gradePhysicalBatch(
		...
		try {
			...
			const rawStudents = (req.body?.students ??
				req.body?.students_json ??
				"") as unknown;
			const nQuestionsRaw = (req.body?.n_questions ??
				req.body?.n ??
				req.body?.num_questions) as unknown;

			let studentsOrdered: number[] = [];
			try {
				if (Array.isArray(rawStudents)) {
					studentsOrdered = (rawStudents as any[])
						.map((v) => Number(v))
						.filter((v) => !isNaN(v));
				} else if (
					...
					studentsOrdered = (parsed as any[])
						.map((v) => Number(v))
						.filter((v) => !isNaN(v));
				}
			} catch (e) {
				res.status(400).json({
					message: "Invalid students list; expected JSON array of IDs",
				});
				return;
			}
			...
			if (
				...
				res
					.status(400)
					.json({ message: "Invalid n_questions; must be between 1 and 55" });
				return;
			}
			...
			const namesAsIds = (() => {
				const raw =
					req.body?.names_as_ids ??
					req.body?.namesAsIds ??
					req.body?.use_names_as_ids;
	...
	async includeStudents(
		...
		try {
			...
			const raw =
				req.body?.student_ids ?? req.body?.students ?? req.body?.studentIds;
			...
			const result = await testService.includeStudentsForTest(
				testId,
				studentIds,
			);
			res.json(result);
	...
	async exportRankings(
		...
		try {
			...
			const { header, rows } =
				await testService.exportCombinedRankingsRows(testIds);
	...
	async updateBubbleAnswers(
		...
		try {
			...
			const rawAnswers = (req.body?.answers ??
				req.body?.answers_map ??
				req.body?.detected_answers) as unknown;
			try {
			...
			const teacherComment =
				typeof req.body?.teacher_comment === "string"
					? req.body.teacher_comment
					: undefined;

			const updated = await testService.updateSubmissionAnswers(
				submissionId,
				answersMap,
				teacherComment,
			);
	...
	async overrideGrade(req: AuthenticatedRequest, res: Response): Promise<void> {
		try {
			...
			const submission = await testService.overrideGrade(
				submissionId,
				score,
				teacher_comment,
			);
	...
	async regradePhysicalSubmission(
		...
		try {
			...
			if (!result.success) {
				res
					.status(400)
					.json({ message: result.message || "Failed to regrade submission" });
				return;
			}
