...
import type { Student, Test, TestAnswer, TestImage } from "../types";
...
class TestService {
	...
	async regradePhysicalSubmission(
		submissionId: number,
		...
		try {
			...
			console.log(
				`[REGRADE] Source image for submission ${submissionId}: ${fullImagePath}`,
			);
			...
			console.log(
				`[REGRADE] Running grading script for submission ${submissionId}`,
			);

			// Delete stale output files before regrade
			const outJsonPre = path.join(outDir, `${testId}-${studentId}.json`);
			const outImgPre = path.join(outDir, `${testId}-${studentId}.jpg`);
			const sameInputAsOutput =
				path.resolve(fullImagePath) === path.resolve(outImgPre);
			try {
				if (fs.existsSync(outJsonPre)) fs.unlinkSync(outJsonPre);
			} catch {}
			try {
				if (sameInputAsOutput) {
					console.warn(
						`[REGRADE] Input image equals output image path; keeping existing file: ${outImgPre}`,
					);
			...
			const exitCode = await new Promise<number | null>((resolve) => {
				...
				child.stdout?.on("data", (data: Buffer) => {
					...
					if (msg) console.log(`[GRADING] ${msg}`);
				});
				child.stderr?.on("data", (data: Buffer) => {
					const msg = data.toString().trim();
					if (msg) console.error(`[GRADING ERROR] ${msg}`);
				});
				child.on("close", (code) => {
					console.log(
						`[REGRADE] Script finished for submission ${submissionId} (exit ${code})`,
					);
					resolve(code);
				});
				child.on("error", (err) => {
					console.error(
						`[REGRADE] Script error for submission ${submissionId}:`,
						err.message,
					);
					resolve(null);
				});
			...
			console.log(
				`[REGRADE] Output status for submission ${submissionId}: json=${fs.existsSync(outJson)} image=${fs.existsSync(outImg)}`,
			);
			...
			try {
				...
				console.error(
					`[REGRADE] Failed to parse output JSON for submission ${submissionId}:`,
			...
			// Update submission
			const persistedSourcePath = (sourceImagePath || imagePath || fullImagePath).replace(
				/\\/g,
				"/",
			);
	...
	> {
		...
		console.log(
			`[GRADING] Starting batch grading for test ${testId} (${studentsOrdered.length} students, ${files.length} files, ${nQuestions} questions)`,
		);
		...
		if (namesAsIds) {
			...
			if (fileIds.length !== providedIds.length) {
				throw new Error(
					`Mismatch: provided ${providedIds.length} students but ${fileIds.length} identifiable files. Ensure each file is named exactly as the student ID.`,
				);
			}
			...
			if (missing.length > 0 || extra.length > 0) {
				throw new Error(
					`ID mismatch. Missing files for students: [${missing.join(", ")}], extra files not in list: [${extra.join(", ")}]`,
				);
			}
		}
		...
		for (let i = 0; i < studentsOrdered.length; i++) {
			...
			if (!fs.existsSync(inputPath)) {
				console.error(
					`[GRADING ERROR] Source image missing for student ${studentId}: ${inputPath}`,
				);
				results.push({
			...
			try {
				...
				// Ignore stale output cleanup errors
			}
			try {
				if (fs.existsSync(outImgPre)) fs.unlinkSync(outImgPre);
			} catch {
				// Ignore stale output cleanup errors
			}
			...
			const exitCode = await new Promise<number | null>((resolve) => {
				...
				child.on("close", (code) => {
					console.log(
						`[GRADING] Script finished for student ${studentId} (exit ${code})`,
					);
					resolve(code);
				});
				child.on("error", (err) => {
					console.error(
						`[GRADING ERROR] Script error for student ${studentId}: ${err.message}`,
					);
					resolve(null);
				});
			});
			...
			if (exitCode !== 0) {
				logger.error(
					`Batch grading script failed for student ${studentId} with exit code ${exitCode}`,
				);
				results.push({
					student_id: studentId as number,
					submission_id: -1,
			...
			const outImg = path.join(outDir, `${testId}-${studentId}.jpg`);
			console.log(
				`[GRADING] Student ${studentId} output status: json=${fs.existsSync(outJson)} image=${fs.existsSync(outImg)}`,
			);
			let detected: Record<string, string> | null = null;
			try {
				const raw = fs.readFileSync(outJson, "utf8");
				...
				console.error(
					`[GRADING ERROR] Failed to parse JSON for student ${studentId}:`,
					parseError instanceof Error ? parseError.message : parseError,
				);
				detected = null;
			}
			...
			if (!detected) {
				console.error(
					`[GRADING ERROR] No valid grading output JSON for student ${studentId}`,
				);
				results.push({
					student_id: studentId as number,
					submission_id: -1,
	...
	async createTest(testData: CreateTestData): Promise<Test> {
		...
		logger.debug(
			`[createTest] Input start_time: ${testData.start_time} -> With TZ: ${startTimeWithTz}`,
		);
		logger.debug(
			`[createTest] Input end_time: ${testData.end_time} -> With TZ: ${endTimeWithTz}`,
		);
    ...
    `;
		...
		if (row) {
			logger.debug(
				`[createTest] DB returned start_time: ${row.start_time}, type: ${typeof row.start_time}, instanceof Date: ${row.start_time instanceof Date}`,
			);
			...
			logger.debug(
				`[createTest] Final: start_time=${row.start_time}, start_time_utc=${row.start_time_utc}, start_time_ms=${row.start_time_ms}`,
			);
		}
		return row;
	}
    ...
    `;
		...
		const withImages = await Promise.all(
			tests.map(async (r) => {
				...
				logger.debug(
					`[getAllTests] Test ${r.id}: raw start_time=${r.start_time}, type=${typeof r.start_time}, instanceof Date=${r.start_time instanceof Date}`,
				);
				logger.debug(
					`[getAllTests] Test ${r.id}: startTimeLocal=${startTimeLocal}, computed UTC=${formatUtc(startTimeLocal)}`,
				);
	...
	async getTestById(testId: number): Promise<
		| (Test & {
				images?: Array<{
					id: number;
					image_path: string;
	...
	): Promise<Array<{ id: number; image_path: string; display_order: number }>> {
	...
	async getEligibleStudentsForTest(testId: number): Promise<
		Array<{
			id: number;
			name: string;
			phone_number: string;
	...
	async includeStudentsForTest(
		testId: number,
		studentIds: number[],
	): Promise<{
		created: Array<{ student_id: number; submission_id: number }>;
		skipped: number[];
	}> {
		...
		for (const sidRaw of studentIds) {
			...
			// Check existing
			const existingQ =
				"SELECT id FROM test_answers WHERE test_id = $1 AND student_id = $2 LIMIT 1";
	...
	async updateTest(
		testId: number,
		testData: Partial<CreateTestData>,
	): Promise<Test | null> {
		// Ensure times have Cairo timezone (+02:00) if no timezone is specified
		// Egypt is always UTC+2 (EET) - stopped observing DST in 2014
		...
		if (processedData.start_time) {
			processedData.start_time = ensureCairoTimezone(
				processedData.start_time,
			) as string;
		}
		if (processedData.end_time) {
			processedData.end_time = ensureCairoTimezone(
		...
		Object.entries(processedData).forEach(([key, value]) => {
			...
			if (
				["test_type", "student_group", "grade", "view_type"].includes(
					mappedKey,
				) &&
				(value === "" || value === null)
			) {
	...
	private async regradeSubmissionsForTest(
		testId: number,
		correctAnswers: any,
	): Promise<void> {
		try {
          ...
          `;
			...
			logger.info(
				`Successfully re-graded ${submissions.length} submissions for test ${testId}`,
			);
		} catch (error) {
	...
	async getAvailableTestsForStudent(studentId: number): Promise<Test[]> {
		try {
			...
			const studentQuery =
				"SELECT grade, student_group FROM students WHERE id = $1";
			...
			logger.debug(
				`Fetching tests for student ${studentId} (grade: ${student.grade}, group: ${student.student_group})`,
			);
      ...
      `;
			...
			const tests = result.rows || [];
			logger.info(
				`Found ${tests.length} available tests for student ${studentId}`,
			);
			...
			return tests.map((test: any) => {
				...
				logger.debug(
					`[getAvailableTests] Test ${test.id}: status=${test.test_status}, raw start_time=${test.start_time}, startTimeLocal=${startTimeLocal}`,
				);
	...
	async getTestQuestions(
		...
		if (hasFinalSubmission) {
			const canView =
				(availableTest as any).view_type === "IMMEDIATE" ||
				(availableTest as any).view_permission === true;
			if (!canView) {
				return null;
			}
		}
		...
		if (fullTest.test_type === "MCQ") {
			...
			(testData as any).start_time_utc = formatUtc(
				(testData as any).start_time,
			);
		...
		if (fullTest) {
			...
			(fullTest as any).start_time_utc = formatUtc(
				(fullTest as any).start_time,
			);
    ...
    `;
		...
		if (
			...
			const correctAnswers =
				typeof testForGrading.correct_answers === "string"
					? JSON.parse(testForGrading.correct_answers)
					: testForGrading.correct_answers;
			score = this.calculateScore(
				answers,
				correctAnswers,
				testForGrading.test_type,
			);
			graded = true;
		}
		...
		if (testResult) {
			...
			try {
				...
				if (
					...
					try {
						testResult._parsed_correct_answers = JSON.parse(
							testResult.correct_answers,
						);
					} catch (e) {
			...
			if (testResult.visible_score !== null) {
				try {
					...
					const ca = testResult.correct_answers_visible; // already parsed sanitized
					...
					if (testType === "MCQ") {
						...
						let correct = 0;
						const stuAnsArr =
							ans && (ans.answers || ans) ? ans.answers || ans : [];
			...
			if (testResult.visible_score !== null && testResult.manual_grades) {
				try {
					if (typeof testResult.manual_grades === "string") {
						testResult.manual_grades_visible = JSON.parse(
							testResult.manual_grades,
						);
					} else {
		...
		if (test.correct_answers) {
			parsedCorrect =
				typeof test.correct_answers === "string"
					? (() => {
							try {
								return JSON.parse(test.correct_answers as any);
							} catch {
								return null;
							}
						})()
		...
		try {
			...
			if (testType === "MCQ") {
				...
				if (submission?.manual_grades) {
					try {
						const mg =
							typeof submission.manual_grades === "string"
	...
	async uploadBubbleSheet(
		...
		try {
			...
			if (existingRes.rows.length > 0) {
				...
				try {
					const ans =
						existing.answers && typeof existing.answers === "string"
	...
	async deleteSubmission(submissionId: number): Promise<boolean> {
		...
		try {
			...
			try {
				const ans =
					submission.answers && typeof submission.answers === "string"
						? JSON.parse(submission.answers)
						: submission.answers;
				if (ans) {
			...
			for (const f of Array.from(new Set(filesToDelete.filter(Boolean)))) {
				try {
					if (typeof f === "string" && f && !f.startsWith("http")) {
						await unlinkAsync(f).catch((err) =>
							logger.error("Failed deleting submission file", f, err),
						);
					}
				} catch (e) {
		...
		try {
			if (testType === "MCQ") {
				...
				correctAnswers.questions.forEach((question: any) => {
					const studentAnswer = (studentAnswers.answers || []).find(
			...
			if (testType === "BUBBLE_SHEET" || testType === "PHYSICAL_SHEET") {
				...
				const correctMap =
					correctAnswers && (correctAnswers.answers || correctAnswers);
				const studentMap =
					studentAnswers && (studentAnswers.answers || studentAnswers);
	...
	async addTestImages(
		images: Array<{ testId: number; imagePath: string; displayOrder: number }>,
	): Promise<TestImage[]> {
      ...
      `;
			...
			let nextOrder = orderResult.rows[0]?.next_order
				...
				: 0; // Fixed: Added nullish coalescing for images[0]
        ...
        `,
			...
			await Promise.all(
				images.map((img) =>
					unlinkAsync(img.imagePath).catch((unlinkError) =>
						logger.error(
							"Failed deleting uploaded image during rollback:",
							img.imagePath,
							unlinkError,
						),
					),
			...
			if (deleteResult.rowCount && deleteResult.rowCount > 0) {
				...
				const deletePromises = getResult.rows.map((row) =>
					unlinkAsync(row.image_path).catch((error) =>
						logger.error(
							`Failed to delete image file: ${row.image_path}`,
							error,
						),
					),
				);
    ...
    `;
		...
		const computeCorrect = (
			testType: string,
			correctAnswersRaw: any,
			studentAnswersRaw: any,
		) => {
			try {
	...
	async exportCombinedRankingsRows(
		testIds: number[],
	): Promise<{ header: string[]; rows: Array<any[]> }> {
	...
	async getStudentRank(
      ...
      `;
			...
			const studentSubmission = submissions.rows.find(
	...
	async regradeAllSubmissions(testId: number): Promise<void> {
		...
		try {
			...
			if (!submissions || submissions.length === 0) {
				logger.info(
					`No submissions found for test ${testId}. Nothing to regrade.`,
				);
				return;
			}
