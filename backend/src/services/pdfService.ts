...
const convertPdfToImages = async (pdfBuffer: Buffer, testId: string) => {
	const requestId = uuidv4();
	console.log(
		`[${requestId}] Starting PDF to image conversion for test ${testId}`,
	);
	...
	try {
		...
		try {
			...
			console.error(
				`[${requestId}] Error cleaning up temporary file:`,
				cleanupError,
			);
		}
	}
...
const savePdf = async (file: Express.Multer.File, testId: string) => {
	ensureDirectories();
...
// Get image path for a specific test page
const getImagePath = (testId: string, pageNumber: number) => {
