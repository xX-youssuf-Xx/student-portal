		...
		return (
			<div className="answers-comparison">
				...
				{correctQuestions.map((q, idx) => {
					...
					const openGradeRaw =
						manualGrades?.[q.id] ?? manualGrades?.[String(q.id)];
					const openGrade =
					...
					const image = testImages?.find(
						(img) => img.display_order === q.media_index,
					);
					const imageUrl = image
						? `${API_BASE}/${image.image_path.replace(/\\/g, "/")}`
						: null;
	...
	return (
		<div className="test-result">
			...
			<div className="result-content">
					...
					<div className="score-card">
						...
						<div className="score-details">
							...
							{(() => {
								...
								return (
									<p style={{ marginTop: 8 }}>
										<strong>الدرجة:</strong> {"( "} {stats.correct} من{" "}
										{stats.total} {" )"} — {"  "}
										{pct}%
									</p>
								);
										...
										>
											...
											>
												{Math.floor(parseFloat(result.average_score))}%{(() => {
													...
													if (total > 0) {
														const avgCorrect = Math.floor(
															(parseFloat(result.average_score) / 100) * total,
														);
														return (
															<span
																style={{
				...
				{showAnswers &&
					result.visible_answers &&
					result.correct_answers_visible && (
						<div className="answers-section">
							{result.test_type === "MCQ" && renderMCQComparison()}
							{result.test_type === "BUBBLE_SHEET" &&
								renderBubbleSheetComparison()}
							{result.test_type === "PHYSICAL_SHEET" &&
