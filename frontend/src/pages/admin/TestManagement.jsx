...
const TestModal = ({ test, onClose, onSave }) => {
	...
	const [formData, setFormData] = useState({
		...
		start_time: test?.start_time
			? isoToDatetimeLocalPreserve(test.start_time)
			: "",
		...
		test_group:
			test?.test_group !== null && test?.test_group !== undefined
				? String(test.test_group)
				: "",
			...
			const testData = {
				...
				correct_answers: {
					...
					answers:
						...
						formData.test_type === "PHYSICAL_SHEET"
											...
											onClick={() => {
												...
												setFormData((prev) => ({
													...
													imageFiles: prev.imageFiles.filter(
														(_, i) =>
															i !==
															index - (prev.images.length - newImages.length),
													),
												}));
											}}
										>
...
const SubmissionsModal = ({ test, submissions, onClose, onGradeUpdate }) => {
	...
	const handleRegradeAll = async () => {
		if (
			!window.confirm(
				"هل أنت متأكد من إعادة تصحيح جميع المشاركات؟ قد تستغرق هذه العملية بعض الوقت.",
			)
		)
			return;
		try {
	...
	const openManualGrading = async (submission) => {
		...
		try {
			const res = await axios.get(
				`/tests/${test.id}/submissions/${submission.id}`,
			);
			...
			const gradesPct = Object.fromEntries(
				Object.entries(existingGrades).map(([k, v]) => [
					k,
			...
			try {
				const imgPath =
					...
					(detail.submission.answers.bubble_image_path ||
						detail.submission.answers.file_path ||
						detail.submission.answers.bubble_image);
				if (imgPath) {
	...
	const handleSaveManualGrades = async () => {
		...
		try {
			...
			const grades = Object.fromEntries(
				Object.entries(gradeData.gradesPct).map(([k, v]) => [
					k,
					Math.max(0, Math.min(1, Number(v) / 100)),
				]),
			);
			...
			await axios.patch(`/submissions/${gradingSubmission}/manual-grades`, {
				grades,
				teacher_comment:
					manualDetail?.test?.test_type === "PHYSICAL_SHEET"
						? null
	...
	const renderManualGradingForm = (detail) => {
		...
		const qData =
			detail.correct_answers && detail.correct_answers.questions
				? detail.correct_answers.questions
				: [];
		const studentAnswers =
			detail.submission?.answers && detail.submission.answers.answers
				? detail.submission.answers.answers
				: [];
		...
		return (
			<div className="grading-form">
							...
							return (
								...
								>
									...
									>
										...
										>
											{isOpen ? "سؤال مفتوح" : "اختيار من متعدد"}
										</span>
									</div>
									<div className="question-body" style={{ marginTop: 8 }}>
										...
										{!isOpen && (
											...
											>
												<div>
													إجابة الطالب:{" "}
													<strong>{stAns ? stAns.answer : "-"}</strong>
												</div>
												<div>
													الإجابة الصحيحة: <strong>{q.correct ?? "-"}</strong>
												</div>
											</div>
										)}
										{isOpen && (
											<div className="open-block" style={{ marginTop: 8 }}>
												...
												<div className="grade-input">
													...
													<input
														...
														value={currentPct}
														onChange={(e) =>
															handleManualGradeChange(q.id, e.target.value)
														}
														placeholder="0 - 100"
													/>
												</div>
				...
				{detail?.test?.test_type !== "PHYSICAL_SHEET" && (
					<div className="form-group" style={{ marginTop: 12 }}>
						...
						<textarea
							...
							onChange={(e) =>
								setGradeData((prev) => ({ ...prev, comment: e.target.value }))
							}
						/>
					</div>
				)}
				...
				<div className="form-actions" style={{ marginTop: 12 }}>
					<button className="btn-secondary" onClick={handleCancelGrading}>
						إلغاء
					</button>
					<button className="btn-primary" onClick={handleSaveManualGrades}>
						حفظ الدرجات
					</button>
				</div>
			</div>
				...
				<div className="submissions-toolbar">
					...
					<div className="toolbar-stats">
						<span className="stat-badge">
							الإجمالي: <strong>{localSubs.length}</strong>
						</span>
						<span className="stat-badge">
							نتائج البحث:{" "}
							<strong>
								{
									localSubs.filter(
										(sub) =>
											searchTerm === "" ||
											(sub.student_name &&
												sub.student_name.includes(searchTerm)),
									).length
								}
							</strong>
						</span>
							...
							.filter(
								(submission) =>
									...
									(submission.student_name &&
										submission.student_name.includes(searchTerm)),
							)
							.map((submission) => (
								<div key={submission.id} className="submission-item">
									<div className="student-info">
										...
										<p>
											الصف:{" "}
											{submission.grade === "1HIGH"
												? "الأول الثانوي"
												: submission.grade === "2HIGH"
													? "الثاني الثانوي"
													: submission.grade === "3HIGH"
														? "الثالث الثانوي"
														: submission.grade}
										</p>
										{submission.student_group && (
											<p>
												المجموعة:{" "}
												{submission.student_group === "A"
													? "أ"
													: submission.student_group === "B"
														? "ب"
														: submission.student_group === "C"
															? "ج"
															: submission.student_group === "D"
																? "د"
																: submission.student_group}
											</p>
										)}
											...
											: (() => {
													...
													return (
														<p className="no-score">
															{stats
																? `صحيح ${stats.correct} من ${stats.total}`
																: "لم يتم التقدير"}
														</p>
													);
												})()}
									...
									<div className="submission-actions-modern">
												...
												<>
													<button
														...
														onClick={async () => {
															if (
																!window.confirm(
																	"هل أنت متأكد من إعادة تصحيح هذه المشاركة؟",
																)
															)
																return;
															try {
																const res = await axios.post(
																	`/submissions/${submission.id}/regrade-physical`,
																);
																if (res.data.success) {
																	alert(
																		`تم إعادة التصحيح بنجاح. الدرجة: ${res.data.score}%`,
																	);
																	...
																	alert(
																		"فشل: " +
																			(res.data.message || "خطأ غير معروف"),
																	);
																}
															} catch (e) {
													...
													<button
														...
														onClick={async () => {
															...
															try {
																...
																const count = detail?.correct_answers?.answers
																	? Object.keys(detail.correct_answers.answers)
																			.length
																	: 50;
										...
										<div className="action-group secondary-actions">
											{((submission.answers &&
												(submission.answers.file_path ||
													submission.answers.bubble_image_path ||
													submission.answers.bubble_image)) ||
												submission.bubble_image_path) && (
												<button
											...
											<button
												...
												onClick={async () => {
													if (
														!window.confirm("هل أنت متأكد من حذف هذه المشاركة؟")
													)
														return;
													try {
									...
									{gradingSubmission === submission.id && (
										<div className="grading-form">
											...
											{!loadingDetail &&
												manualDetail &&
												renderManualGradingForm(manualDetail)}
										</div>
									)}
										...
										<div className="grading-form" style={{ marginTop: 10 }}>
											...
											<div className="bubble-answers-grid">
												{Array.from(
													...
													const correctAnswer =
														answersDetail?.correct_answers?.answers?.[qnum] ||
														"";
											...
											<div className="form-actions" style={{ marginTop: 8 }}>
												<button
													className="btn-secondary"
													onClick={() => setEditingAnswersId(null)}
												>
													إلغاء
												</button>
												<button
													...
													onClick={async () => {
														try {
															await axios.patch(
																`/submissions/${submission.id}/answers`,
																{
																	answers: answersMap,
																},
															);
	...
	const handleSubmit = async () => {
		try {
			...
			if (bad.length > 0) {
				...
				if (
					!window.confirm(`بعض الملفات لا تحتوي على أرقام في أسمائها:
${list}
هل تريد المتابعة؟ سيتم التحقق بدقة في السيرفر.`)
				)
					return;
			}
			setSubmitting(true);
			...
			await axios.post(`/tests/${test.id}/grade-physical-batch`, form, {
				headers: { "Content-Type": "multipart/form-data" },
			});
			...
			<div className="modal-content">
				...
				<div className="form-grid">
					<div className="form-group">
						...
						<input
							...
							value={nQuestions}
							onChange={(e) =>
								setNQuestions(parseInt(e.target.value || "0", 10) || 0)
							}
						/>
					</div>
					<div className="form-group">
						...
						/>
						<small>
							سيتم مطابقة كل صورة تلقائياً بالطالب الذي يطابق رقم اسم الملف
						</small>
						{files.length > 0 && (
							<div style={{ marginTop: 6 }}>تم اختيار {files.length} صورة</div>
						)}
					</div>
				</div>
				<div className="section-header">
					...
					>
						<div style={{ width: 32 }}>
							<input
								type="checkbox"
								checked={
									selectedSet.size === ordered.length && ordered.length > 0
								}
				...
				<div className="form-actions" style={{ marginTop: 10 }}>
					...
					<button
						...
						disabled={submitting}
					>
						{submitting ? "جارٍ التصحيح..." : "بدء التصحيح"}
					</button>
				</div>
			</div>
		</div>
...
const IncludeStudentsModal = ({ test, onClose, onDone }) => {
	...
	const filteredStudents = students.filter(
		(s) =>
			String(s.name || "")
				.toLowerCase()
				.includes(includeQuery.toLowerCase()) ||
			String(s.id).includes(includeQuery),
	);
	...
	const handleInclude = async () => {
		...
		try {
			...
			alert(
				`تم إضافة ${created.length} طلاب، تم تخطي ${skipped.length} (لأن لديهم بالفعل مشاركات)`,
			);
			onDone && onDone();
		} catch (e) {
			...
			<div className="modal-content">
				...
				<div style={{ padding: 12 }}>
					{loading ? (
						...
						<p>لا يوجد طلاب مؤهلين لهذا الاختبار</p>
					) : (
						<div>
									...
									>
										...
										<div style={{ marginInlineStart: 8, flex: 1 }}>
											...
											<div style={{ fontSize: 12, color: "#666" }}>
												ID: {s.id} — {s.phone_number}
											</div>
										</div>
										<div style={{ minWidth: 120, textAlign: "right" }}>
											{translateGrade(s.grade)}
											{s.student_group
												? ` — ${translateGroup(s.student_group)}`
												: ""}
										</div>
									</div>
								))}
				...
				<div className="form-actions" style={{ padding: 12 }}>
					...
					>
						{submitting ? "جارٍ الإضافة..." : "إضافة الطلاب"}
					</button>
				</div>
			</div>
		</div>
...
const GradeOverrideModal = ({ submission, test, onClose, onSave }) => {
	...
	const handleCorrectAnswersChange = (newCorrect) => {
		...
		const newScore =
			totalQuestions > 0
				? Math.round((numCorrect / totalQuestions) * 10000) / 100
				: 0;
		...
		<div className="modal-overlay">
			<div className="modal-content" style={{ maxWidth: 500 }}>
				...
				<div style={{ padding: "20px" }}>
					<div className="form-group">
						<label>
							إجمالي عدد الأسئلة: <strong>{totalQuestions}</strong>
						</label>
					</div>
...
const ExportModal = ({ tests, onClose }) => {
	...
	useEffect(() => {
		setFiltered(
			tests.filter((t) => t.title.toLowerCase().includes(query.toLowerCase())),
		);
	}, [query, tests]);
	...
	const handleExport = async () => {
		if (!selected.size) {
			alert("يرجى اختيار اختبار واحد على الأقل");
			return;
		}
		try {
			setExporting(true);
			const body = { test_ids: Array.from(selected) };
			const res = await axios.post("/tests/export-rankings", body, {
				responseType: "blob",
			});
						...
						{filtered.map((t) => (
							...
							>
								...
								<div style={{ marginInlineStart: 8 }}>
									{t.title} — {t.grade}
									{t.student_group ? ` — ${t.student_group}` : ""}
								</div>
							</div>
						))}
						{filtered.length === 0 && (
